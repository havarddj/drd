...
intrinsic NarrowClassField(D) -> Any
{Compute narrow class field of Q(sqrt D)}
    F := QuadraticField(D);
    
    return NumberField(RayClassField(Integers(F)*1,[1,2]));
...
intrinsic Diagonal_Restriction_data(F,m) -> Any // weight 2k
...
function diagonal_restriction_derivative(F,p,m,Fs,Forms : pprec := m)
    /* This is the overconvergent modular form G_tau = G_psi/2 from
    DPV2 Thm. 4.8, where tau is the largest root of the one-variable
    polynonmial attached to  Q */
	...
	Diag_F := Diag_F + coeff_n*q^n; /* Cf DPV2 eq 80 */
    end for;
    print "computed diagonal restriction";
    print "finding overconvergent form";
    ct := IsOverconvergent(Diag_F : prt := true, f:= f);
    print ct;
    return R!ct + Diag_F;
...
intrinsic Diagonal_Restriction_Derivative(F::QuadBinElt,p::RngIntElt,m::RngIntElt : pprec := m) -> Any
{Compute diagonal restriction derivative wrt indefinite quadratic form F, prime p and number of terms m.}    
...
intrinsic Diagonal_Restriction(F,k,m : Stabilisation := 1, pprec := m) -> Any
{Compute diagonal restriction of Hilbert Eisenstein series attached to F of parallel weight (k,k)}
...
intrinsic Diagonal_Restriction_Naive(chi, k, m : stab := 1) -> Any
{Compute diagonal restriction of hilbert eisenstein series attached to chi}
    F := NumberField(BaseField(AbelianExtension(chi)));
    O := Integers(F);
    sqrtD := O!F.1;
    assert Discriminant(F) mod ZZ!(sqrtD^2) eq 0;
    ...
    for n := 1 to m do
	an := 0;
	for b in [-Floor(n*sqrtD)..Floor(n*sqrtD)] do
	    nu := ((b + n*sqrtD)/(2*sqrtD));

	    assert Trace(nu) eq n;
	    assert [u gt 0 : u in RealEmbeddings(nu)] eq [true, true]; 
	    if nu in Different(O)^-1 then 
		I := ideal< O | nu*Different(O)>;
		Divs := Divisors(I);
		if #Divs gt 1 then
		    for J in Divs do
			/* print GCD(J,stab*O) eq 1*O; */
			if GCD(J,stab*O) eq 1*O then
			    an +:= chi(J)*Norm(J)^k;
			end if;
		    end for;
		end if;
